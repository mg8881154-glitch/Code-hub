...
import { Code2, User, Mail, Lock, ArrowRight, AlertCircle, Sparkles, CheckCircle2 } from 'lucide-react'
...
export default function Signup() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setError(err.response?.data?.message || 'Signup failed. Please try a different username or email.')
    } finally {
      setLoading(false)
    }
  }
    ...
    <div className={`min-h-screen flex items-center justify-center px-4 sm:px-6 py-12 transition-colors relative overflow-hidden ${
      ...
      <div className="w-full max-w-md relative z-10">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-2xl font-black tracking-tight">Create Free Account</h1>
          <p className={`text-xs mt-1.5 ${dark ? 'text-slate-400' : 'text-slate-600'}`}>
            Join thousands of developers mastering DSA and technical interviews
          </p>
        </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              ...
              <div className="relative">
                <User className={`w-4 h-4 absolute left-3.5 top-1/2 -translate-y-1/2 ${dark ? 'text-slate-500' : 'text-slate-400'}`} />
                <input
                  type="text"
                  required
